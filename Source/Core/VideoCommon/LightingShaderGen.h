...
static void GenerateLightShader(T& object, LightingUidData& uid_data, int index, int litchan_index, const char* lightsColName, const char* lightsName, int coloralpha)
{
	...
	if (!(chan.attnfunc & 1))
	{
		...
		switch (chan.diffusefunc)
		{
			...
			case LIGHTDIF_CLAMP:
				...
				object.Write("lacc.%s += int%s(round(%sdot(ldir, _norm0)) * " LIGHT_COL"));\n",
	...
	else // spec and spot
	{
		...
		{
			case LIGHTDIF_NONE:
				object.Write("lacc.%s += int%s(round(attn * " LIGHT_COL"));\n", swizzle, swizzle_components, LIGHT_COL_PARAMS(lightsColName, index, swizzle));
				break;
			case LIGHTDIF_SIGN:
			case LIGHTDIF_CLAMP:
				object.Write("lacc.%s += int%s(round(attn * %sdot(ldir, _norm0)) * " LIGHT_COL"));\n",
